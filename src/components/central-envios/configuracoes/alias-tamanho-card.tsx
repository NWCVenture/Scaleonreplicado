...
// CRUD de aliases de tamanho. Tabela + dialog de edição + alert de delete.
// `modeloId = null` = alias global da conta.
...
export function AliasTamanhoCard({
  aliases,
  modelosDisponiveis,
  loading,
  erro,
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0">
        <div>
          <CardTitle>Aliases de tamanho</CardTitle>
          <CardDescription>
            Traduz códigos alternativos de tamanho (ex.: EXG → EGG) antes da
            normalização. Escopo global ou por modelo.
          </CardDescription>
        </div>
        {podeEditar && (
      ...
      <Dialog open={editando !== null} onOpenChange={(v) => !v && fechar()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editando === "novo" ? "Novo alias de tamanho" : "Editar alias"}
            </DialogTitle>
          </DialogHeader>
