...
const FROM = process.env.RESEND_FROM_EMAIL || "NWC ERP <noreply@resend.dev>";
...
export function buildVerificationEmailHtml(url: string, userName: string) {
  return `
    <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px">
      <h2 style="color:#f97316">NWC ERP — Verifique seu email</h2>
      <p>Olá, <strong>${userName}</strong>!</p>
      <p>Clique no botão abaixo para verificar seu endereço de email e garantir acesso contínuo ao sistema.</p>
      <a href="${url}"
        style="display:inline-block;margin:24px 0;padding:12px 28px;background:#f97316;color:#fff;border-radius:6px;text-decoration:none;font-weight:bold">
        Verificar Email
      </a>
